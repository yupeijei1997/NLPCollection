...
def bulid_dataset(dataset_path: str, tokenizer, pad_size):
    ...
    """
    datasetpkl = dataset_path + "/dataset.pkl"  # dataset_path路径下的dataset.pkl文件，即一个路径
    # 加载数据集
...
def save_model(args, model, optimizer, total_batch):
    # Save model checkpoint
    # 要保存的模型的路径与名称格式
    output_dir = os.path.join(args.output_dir, "ckpt-{}".format(total_batch))
    # 不存在则创建
